...
class ModemPoolScaner(object):
    ...
    def start(self,):
        ...
        for i in xrange(1,256):
            try:
                logger.info("start detect %s",'COM{}'.format(i))
                modem = SerialComms(port='/dev/ttyACM{}'.format(i),baudrate=115200)
                modem.connect() 
                modem.close()
                available_port.append( '/dev/ttyACM{}'.format(i))
            except Exception,e:
                if modem.alive:
                    modem.close()
            
        threads = []
        logger.info('available_port %s'%available_port)
    ...
    def detecter(self,port):
        ...
        for ba in BAUDRATES:
            try:
                ...
                logger.info("unavailable %s,%s,%s",port,ba,e)
                if modem.alive:
                    modem.close()  
...
if __name__ == '__main__':
    ...
    try:
        modem = SerialComms(port='/dev/ttyACM31', baudrate=115200, )
        modem.connect()
        a = modem.write('AT+CPAS\r', timeout=3)  # 初始化
        modem.close()
    except Exception, e:
