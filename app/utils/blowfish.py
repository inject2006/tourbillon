...
class Blowfish:
    ...
    def encryptCTR(self, data):
        ...
        estr = "".join(result)
        return base64.b64encode(estr)

    def decryptCTR(self, data):

        self.ctr_iv = 0
        self._calcCTRBUF()
        crypted = base64.b64decode(data)
        result = []
        for ch in crypted:
            result.append(chr(ord(ch) ^ self._nextCTRByte()))
        estr = "".join(result)
...
if __name__ == '__main__':
    ...
    # text = "SHA1541118000000SZX1541127300000FM9331HYJ_ECB1470.0/321312/32131.0"
    # print "\tText:\t\t", text
    # crypted = Blowfish().encryptCTR(text)
    # print "\tEncrypted:\t", repr(crypted)
    text = 'OzEza0plhPZ1zSf/EEBqRt2HIdJ6C109VyM7rYxLpys5Pg0KwSERmj/3w6NlhklBKEJBjNtnzNZYcND1ERvlAiBUIw/onU0y2qVI'
    decrypted = Blowfish().decryptCTR(text)
    print "\tDecrypted:\t", decrypted

    import time
    aa = time.time()
    for x in range(0,122):
        a = Blowfish().encryptCTR('SHA1539223200000HRB1539237600000MU222HYJN|||1200.0|60.0||600.0|0.0||0.0|0.0')
