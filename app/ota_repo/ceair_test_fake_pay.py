...
class CeairTestFakePay(OTABase):
    """
    OTA 接口基类，
    ...
    ota_name = 'ceair_test_fake_pay'  # OTA名称，保证唯一，必须赋值
    ...
    cn_ota_name = '东航测试OTA（假支付）'

    def __init__(self):
        super(CeairTestFakePay, self).__init__()
    ...
    def _before_search_interface(self, req_body):
        ...
        self.search_info.ret_date = req_body['retDate'] if req_body['retDate'] else None
    ...
    "routing":{
        ...
        req_body = json.loads(req_body)
        first_segment = req_body['routing']['fromSegments'][0]
        last_segment = req_body['routing']['fromSegments'][-1]

        self.search_info.from_date = datetime.datetime.strptime(first_segment['depTime'], '%Y%m%d%H%M').strftime('%Y-%m-%d')
        self.search_info.from_airport = first_segment['depAirport']
        self.search_info.to_airport = last_segment['arrAirport']
    ...
    def _before_order_interface(self, req_body):
        ...
        first_segment = req_body['routing']['fromSegments'][0]
        last_segment = req_body['routing']['fromSegments'][-1]
        self.order_info.from_date = datetime.datetime.strptime(first_segment['depTime'], '%Y%m%d%H%M').strftime('%Y-%m-%d')
        self.order_info.from_airport = first_segment['depAirport']
        self.order_info.to_airport = last_segment['arrAirport']
