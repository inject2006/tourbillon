...
class PrivateOta(OTABase):
    """
    自运营OTA
    """
    ticket_process_mode = 'api'  # 票号处理模式-接口方式
    order_detail_process_mode = 'api'  # 订单详情同步模式-接口模式
    ota_name = 'private_ota'  # OTA名称，保证唯一，必须赋值
    ota_env = 'PROD'  # 生产OR测试接口 TODO 暂时无作用
    ota_token = '7nhzq28ruvkxftgs'  # ota 访问验证token
    pay_channel = 'ALIPAY'
    cn_ota_name = '自有OTA'
    verify_search_timeout = 30  # 验价搜索时间
    order_search_timeout = 30  # 订单搜索超时时间



    def __init__(self):
        super(PrivateOta, self).__init__()
    ...
    def _before_order_interface(self, req_body):
        ...
        for pax in req_body['passengers']:
            ...
            pax_info.age_type = pax_info.current_age_type(from_date=self.order_info.from_date,is_aggr_chd_adt=True)
