...
class FakeOTA(OTABase):
    ...
    order_detail_process_mode = 'polling'  # 订单详情同步模式-接口模式
    #TEST
    ticket_process_mode = 'api'  # 票号处理模式-接口方式
    cn_ota_name = '测试OTA'
    ota_name = 'fakeota'  # OTA名称，保证唯一，必须赋值
    ota_env = 'PROD'  # 生产OR测试接口 TODO 暂时无作用
    ota_token = '123456'  # ota 访问验证token
    product_type = 1  # 运价类型：0: GDS公布运价 1:GDS私有运价 2:航司官网产品  3:廉价航司产品 4:特价产品
    ...
    low_price_provider_channel = 'fakeprovider_test16'
    ...
    "routing":{
        ...
        # self.search_info.ota_product_code = 2
        trip_type = req_body['tripType']
        if trip_type == "1":
            trip_type = 'OW'
        elif trip_type == '2':
        ...
        self.search_info.trip_type = trip_type
        # 是否允许降舱
        self.search_info.allow_cabin_downgrade = req_body.get('allowCabinDownGrade',0)
    ...
    def _before_order_interface(self, req_body):
        ...
        self.order_info.allow_cabin_downgrade = int(req_body.get('allowCabinDownGrade', 0))
        self.order_info.is_test_order = req_body.get('is_test_order', 0)  # 是否为测试订单
        self.order_info.ota_order_id = req_body.get('OtaOrderId', '')
        # self.order_info.ota_product_code = 2
        trip_type = req_body['tripType']
        if trip_type == "1":
            trip_type = 'OW'
        elif trip_type == '2':
        ...
        for pax in req_body['passengers']:
            ...
            if pax_info.used_card_type == 'NI':
                pax_info.card_ni = pax['cardNum']
                pax_info.used_card_no = pax['cardNum']
            else:
                pax_info.used_card_no = pax['cardNum']
                pax_info.used_card_type = 'PP'
                pax_info.card_pp = pax['cardNum']
    ...
    def _after_order_interface(self, order_info):
        ...
        for flight_segment in order_info.routing.ret_segments:
            ...
            output_routing['rule']['baggageInfo']['baggageRules'].append(baggage_info)
        ret['routing'] = output_routing
        Logger().debug('aes_decrypt data %s' % ret)
        ret = self.aes_encrypt(json.dumps(ret))
        self.final_result = ret
        return True
